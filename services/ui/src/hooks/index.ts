...
import {
  ...
  MessageResponse,
  PaginatedResponse,
} from "@repo/client/src";
import throttle from "lodash/throttle";
import { bufferToImageUrl } from "@repo/client/src";
    ...
    (forceLoad = false) => {
      if (profile.isProfilePictureLoaded === false || forceLoad) {
        BetroApiObject.account.fetchProfilePicture().then(async (resp) => {
          dispatch(
            profilePictureLoaded(resp == null ? null : bufferToImageUrl(resp))
          );
        });
      }
    },
...
export function useFetchUserSettings() {
  ...
  const [settings, setUserSettings] =
    useState<Array<UserSettingResponse> | null>(null);
...
export const useFetchUserInfoHook = (
  ...
  const [userInfo, setUserInfo] = useState<UserInfo | null>(
    state == null
      ...
      : {
          ...
          ...state,
          profile_picture:
            typeof state.profile_picture == "string"
              ? null
  ...
  const fetchPosts = useCallback(
    async (forceRefresh = false) => {
      const after =
        pageInfo == null || forceRefresh ? undefined : pageInfo.after;
  ...
  const followHandler = useCallback(
    async (public_key?: string | null) => {
      if (id != null && followee_key_id != null) {
        const follow = BetroApiObject.follow.followUser(
          id,
          followee_key_id,
          public_key
        );
...
export const useApproveUser = (
  ...
  const approveHandler = useCallback(
    (allowProfileRead = false) => {
      if (
        group !== undefined &&
        approval.public_key != null &&
        approval.own_key_id != null
      ) {
...
export const useReadNotification = (notification: NotificationResponse) => {
  ...
  const readNotification = useCallback(async () => {
    const isRead = await BetroApiObject.notifications.readNotification(
      notification.id
    );
    if (isRead) {
      setRead(true);
...
export const useFetchHomeFeed = createFeedHook(
  BetroApiObject.feed.fetchHomeFeed
);
export const useFetchOwnFeed = createFeedHook(
  BetroApiObject.feed.fetchOwnPosts
);
...
export function useFetchConversations() {
  const [response, setResponse] =
    useState<PaginatedResponse<ConversationResponseBackend> | null>(null);
...
export function useFetchMessages(
  ...
  const [response, setResponse] =
    useState<PaginatedResponse<MessageResponse> | null>(null);
  const after = response == null ? undefined : response.after;
  // const [loaded, setLoaded] = useState<boolean>(false);
  const dispatch = useDispatch();
  const getResponse = useCallback(
...
export function useSendMessage(
  ...
  const getResponse = useCallback(
    async (text_content: string) => {
      ...
      if (resp !== null) {
        dispatch(
          addMessage(conversation_id, { ...resp, message: text_content })
        );
      }
    },
...
export function useProcessIncomingMessage() {
  ...
  const processMessage = useCallback(
    async (messageResponse: MessageResponse) => {
      const { sender_id, conversation_id, id, message, created_at } =
        messageResponse;
      let conversation: ConversationResponseBackend | null | undefined =
        conversations.data.find((a) => a.id === conversation_id);
      if (conversation == null) {
        conversation = await BetroApiObject.conversation.fetchConversation(
          conversation_id
        );
        if (conversation != null) {
          dispatch(addConversation(conversation));

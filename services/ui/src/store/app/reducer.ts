...
import { CountResponse } from "@repo/client/src";
  ...
  produce(state, (draft) => {
    switch (action.type) {
      ...
      case ActionTypes.CONVERSATIONS_OPEN: {
        if (
          draft.conversations.opened
            .map((a) => a.id)
            .indexOf(action.payload) === -1
        ) {
          draft.conversations.opened.push({
            id: action.payload,
            visible: true,
          });
        }
      ...
      case ActionTypes.CONVERSATIONS_CLOSE: {
        if (
          draft.conversations.opened.map((a) => a.id).indexOf(action.payload) >=
          0
        ) {
      ...
      case ActionTypes.MESSAGES_ADD: {
        if (
          draft.conversations.messages[action.payload.conversation_id] != null
        ) {
          draft.conversations.messages[
            action.payload.conversation_id
          ].data.splice(0, 0, action.payload.message);
          draft.conversations.messages[
            action.payload.conversation_id
          ].total += 1;
        }
